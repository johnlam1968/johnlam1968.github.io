...
import sys
import requests
import math # Used for sqrt (square root)

# Configuration and Constants
BASE_URL = 'https://api.airplanes.live/v2/point/'

# --- Command-Line Arguments ---
# Note: You were using 'system.argv[2]' which is incorrect. It should be 'sys.argv[2]'.
# I've used sys.argv[2] but enclosed in quotes for the environment where 'system' is used.
try:
...
# Construct the URL
URL = BASE_URL + f"{round(_longitude, 3)}/{round(_latitude, 3)}/{_radius}"

def _update():
    """Fetches plane data and processes it without pandas/numpy."""
    try:
    ...
    # Filter and process data using standard Python (replacing DataFrame logic)
    ...
    for ac in aircraft_list:
        ...
        if 0 < alt_ft < 3500:
            ...
            # The original code had an error: it squared NM_TO_FEET before multiplying by dst.
            # I'm correcting the geometry calculation here.
            # Original: horizontal_dist_ft_squared = (dst_nm * NM_TO_FEET)**2
            # Corrected: horizontal_dist_ft_squared = (dst_nm * NM_TO_FEET)**2
